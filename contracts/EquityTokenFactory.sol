...
contract EquityTokenFactory /* is ERC20Interface, ERC777 Interface */ {
    ...
    require((_amount % ArtifactEquityToken.granularity == 0), "unable to modify token balances at this granularity");
    _;
    }
...
//-----EquityTokenFactory-----------------------------------------------------------------------------------------------------------------  
  ...
  function _createEquityToken(uint _tokenId, bytes32 _companyName, bytes32 _tokenTicker, uint _granularity, uint _totalamount) internal {
  
  ArtifactEquityToken = EquityToken(_tokenId, _companyName, _tokenTicker, _granularity, _totalamount, msg.sender, [msg.sender, governmentAddress]);
...
//-----EquityToken------------------------------------------------------------------------------------------------------------------------
  ...
  //@notes: ERC20 optional, ERC777 mandatory
  function name() public view returns(bytes32) {
    return ArtifactEquityToken.companyName;
  }

  //@notes: ERC20 optional, ERC777 mandatory
  function symbol() public view returns(bytes32) {
    return ArtifactEquityToken.tokenTicker;
  }

  //@dev: smallest part of token not divisible
  //@notes: ERC777 mandatory
  function granularity() public view returns(uint) {
    return ArtifactEquityToken.granularity;
  }
  ...
  //@notes: ERC20 mandatory, ERC777 mandatory
  ...
  function totalSupply() public view returns(uint) {
    return ArtifactEquityToken.totalamount;
  }
    //@dev: returns Infos of equity token, as struct is not returnable in current solidity version
  	function getInfosEquityToken() public view returns (uint, bytes32, bytes32, uint, uint) {
    	return (ArtifactEquityToken.tokenId, ArtifactEquityToken.companyName, ArtifactEquityToken.tokenTicker, ArtifactEquityToken.granularity,
...
//-----TokenTransactions------------------------------------------------------------------------------------------------------------------
  /* This section determines transaction protocolls of the issued equity token
    ...
    function doSend(address _from, address _to, uint256 _txamount, bytes _userData, address _operator, bytes _operatorData, bool _preventLocking) 
    ...
    function send(address _to, uint256 _txamount) public {
        doSend(msg.sender, _to, _txamount, "", msg.sender, "", true);
    }
    function send(address _to, uint256 _txamount, bytes _userData) public {
...
//-----TokenGovernance-------------------------------------------------------------------------------------------------------------------- 
    ...
    function defaultOperators() public view returns(address[2]) {
      return ArtifactEquityToken.defaultOperator;
    }
...
//-----ERC20BackwardCompatibility---------------------------------------------------------------------------------------------------------
    ...
    ///@notice ERC20 mandatory
    function allowance(address _owner, address _spender) public view erc20 returns(uint256 remaining) {
